...
//   task =>
//     task.status === 'inProgress' && task.date === `${year}-${month + 1}-${day}`
// ).length;
// const inprogressByMonth = tasks.filter(
//   task =>
//     task.status === 'inProgress' && task.date.startsWith(`${year}-${month + 1}`)
// ).length;
...
//     // Оновлюємо дані графіка для попереднього місяця
//     updateChartDataForPreviousMonth(prevMonthDate);
//     prevHandler();
//   };
...
//     // Оновлюємо дані графіка для наступного місяця
//     updateChartDataForNextMonth(nextMonthDate);
//     nextHandler();
//   };
